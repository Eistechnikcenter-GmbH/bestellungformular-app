...
function mergeAvailableTagNames(
  ...
  for (const tag of crmTags) names.add(str(tag.name));
  for (const cat of partnerCategories) names.add(str(cat.name));
...
export async function fetchAdAuswertung(
  ...
  const tagNameById = new Map(tags.map((t) => [t.id, str(t.name)]));
  const categoryNameById = new Map(partnerCategories.map((c) => [c.id, str(c.name)]));
  ...
  const partnerNameById = new Map(
    matchPartners.map((p) => [p.id, str(p.name)])
  );
      ...
      if (leasingKunde) {
        ...
        if (crmMatchByName) {
          ...
          const match = findBestPartnerMatch(
            leasingKunde,
            matchPartners.map((p) => ({ id: p.id, name: str(p.name) })),
            FUZZY_MATCH_THRESHOLD
          );
          if (match) {
