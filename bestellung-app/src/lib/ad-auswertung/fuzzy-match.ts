...
function scoreSegment(segment: string, candidate: string): number {
...
export function scorePartnerMatch(leasingText: string, partnerName: string): number {
  const a = normalize(leasingText);
  const b = normalize(partnerName);
  if (a && b && a === b) return 1;

  const segments = leasingText
    .split(/[,;]/)
    .map((s) => s.trim())
    .filter(Boolean);

  if (segments.length === 0) return scoreSegment(leasingText, partnerName);

  let best = 0;
  for (const segment of segments) {
    best = Math.max(best, scoreSegment(segment, partnerName));
  }
  best = Math.max(best, scoreSegment(leasingText, partnerName));
  return best;
}
...
export function extractLeasingSearchTerms(leasingText: string): string[] {
