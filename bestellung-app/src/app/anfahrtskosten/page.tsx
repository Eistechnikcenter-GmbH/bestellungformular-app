...
export default function AnfahrtskostenPage() {
  ...
  useEffect(() => {
    try {
      const cachedManual = window.localStorage.getItem(STORAGE_MANUAL_KEY);
      if (cachedManual) setManualState(JSON.parse(cachedManual) as ManualState);
    } catch {
      // ignore bad cache
    }
    try {
      const cachedCustomer = window.localStorage.getItem(STORAGE_CUSTOMER_KEY);
      if (cachedCustomer) setCustomerState(JSON.parse(cachedCustomer) as CustomerState);
    } catch {
      // ignore bad cache
    }
  }, []);
  ...
  useEffect(() => {
    window.localStorage.setItem(STORAGE_MANUAL_KEY, JSON.stringify(manualState));
  }, [manualState]);

  useEffect(() => {
    window.localStorage.setItem(STORAGE_CUSTOMER_KEY, JSON.stringify(customerState));
  }, [customerState]);
  ...
  useEffect(() => {
    if (activeTab !== "manuell") return;
    const q = manualState.manualAddressQuery.trim();
    if (q.length < 3) {
    ...
    const t = window.setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/anfahrtskosten/suggest?q=${encodeURIComponent(q)}`,
          { cache: "no-store" }
        );
        const json = (await res.json()) as
          | Array<{ label: string; address: string }>
        ...
        if (!res.ok) {
          throw new Error(
            "error" in json ? json.error ?? "Adressvorschläge konnten nicht geladen werden." : "Adressvorschläge konnten nicht geladen werden."
          );
        }
  ...
  }, [activeTab, manualState.manualAddressQuery]);
